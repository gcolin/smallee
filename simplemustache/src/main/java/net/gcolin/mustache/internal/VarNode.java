...
public class VarNode extends Node {
	...
	private Var createVar(Object obj, boolean filter) {
		...
		if (filter && filters != null && filters.length > 0) {
			newVar = new FilterVar(obj.getClass(), filters[filters.length - 1]);
			for (int i = filters.length - 2; i >= 0; i--) {
				newVar = new FilterChainVar(filters[i], newVar);
			}
		} else if (obj.getClass() == String.class) {
			newVar = new StringVar();
		} else if (obj instanceof WrapperFunction) {
			newVar = new WrapperVar(obj.getClass());
		} else {
			newVar = new Var(obj.getClass());
		}
		return newVar;
	}
	...
	private class WrapperVar extends Var {
		...
		public void execute(Context ctx, Object obj) {
			synchronized (this) {
				if (this.wrapper == null || obj != this.wrapper.get()) {
					this.wrapper = new BiFunctionWrapper(null, (WrapperFunction) obj, new CompileContext());
				}
			}
