...
/**
 * Session scoped builder.
 * 
 * @author Gaël COLIN
 * @since 1.0
 */
public class SessionProviderBuilder implements ProviderBuilder {

  private Class<? extends Annotation> scope;

  public SessionProviderBuilder(Class<? extends Annotation> scope) {
    this.scope = scope;
  }
  ...
  public <T> AbstractProvider<T> create(Class<?> type, Type genericType, Class<?> resolvedClazz,
    ...
    return new SessionProvider<T>((Class<T>) type, genericType, (Class<T>) resolvedClazz,
        resolvedGenericType, env);
  }

  @Override
  public <T> AbstractProvider<T> decorate(AbstractProvider<T> provider) {
    return new SessionDecorateProvider<>(provider);
  }
