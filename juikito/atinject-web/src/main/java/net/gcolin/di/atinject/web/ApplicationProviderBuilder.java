...
/**
 * Application scoped provider builder.
 * 
 * @author Gaël COLIN
 * @since 1.0
 */
public class ApplicationProviderBuilder implements ProviderBuilder {

  Class<? extends Annotation> scope;
  
  public ApplicationProviderBuilder(Class<? extends Annotation> scope) {
    this.scope = scope;
  }
  ...
  public <T> AbstractProvider<T> create(Class<?> type, Type genericType, Class<?> resolvedClazz,
    ...
    return new ApplicationProvider<T>((Class<T>) type, genericType, (Class<T>) resolvedClazz,
        resolvedGenericType, env);
  }

  @Override
  public <T> AbstractProvider<T> decorate(AbstractProvider<T> provider) {
    return new ApplicationDecorateProvider<>(provider);
  }
