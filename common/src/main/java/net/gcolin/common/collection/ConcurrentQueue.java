...
/**
 * A concurrent queue implementation FILO (First in Last out) based on an array
 * with a limited size
 * 
 * @since 1.0
 * @author Gaël COLIN
...
public class ConcurrentQueue<E> extends AbstractCollection<E> implements Queue<E> {
	...
	public <T> T[] toArray(T[] array) {
		lock.lock();
		try {
			T[] tmp = array.length >= size ? array : (T[]) Array.newInstance(array.getClass().getComponentType(), size);
