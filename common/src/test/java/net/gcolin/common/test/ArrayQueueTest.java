...
public class ArrayQueueTest {
  ...
  public void testSimple() {
    ArrayQueue<Long> nb = new ArrayQueue<>();
    nb.offer(1L);
    nb.remove(1L);
    Assert.assertTrue(nb.isEmpty());
    ...
    for (int j = 99; j >= 0; j--) {
      Assert.assertEquals(j, nb.poll().intValue());
    }
  }
  ...
  public void testSpeed() {
    ArrayQueue<Long> queue = new ArrayQueue<>();
    Time.tick();
  ...
  public void testRemove() {
    ...
    Assert.assertEquals(3L, queue.get(1).longValue());
