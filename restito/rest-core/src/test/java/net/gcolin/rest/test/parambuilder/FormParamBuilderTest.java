 ...
 * @since 1.0
 */
public class FormParamBuilderTest {

  FormParamBuilder builder;
  ...
  public void before() {
    ...
    builder = new FormParamBuilder(providers);
  }

  @Test
  public void multipartTest() throws IllegalArgumentException, IllegalAccessException, IOException {
    try {
      builder.build(Bean.class, Bean.class, null, true, null);
      Assert.fail();
    ...
    for (Field field : Bean.class.getDeclaredFields()) {
      if (field.isAnnotationPresent(FormParam.class)) {
        ...
        array[idx++] = builder.build(field.getType(), field.getGenericType(),
            field.getAnnotations(), false, field.getAnnotation(FormParam.class));
      }
    }
