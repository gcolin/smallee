...
public class ResourceBuilder {
    ...
    while (clazz != Object.class && clazz != null) {
      for (Method m : clazz.getDeclaredMethods()) {
        ...
        if (!httpMethods.isEmpty() && !methodsWithResources.contains(getMethodKey(m))) {
          resources.addAll(buildResources(m, router, env, component, methodsWithResources,
              componentClass, httpMethods));
        }
      }
   ...
   * @return the resources extracted.
   */
  public List<AbstractResource> buildResources(Method method, Router<ResourceArray> router,
      Environment env, Object singleton, Set<String> methodsWithResources, Class<?> clazz,
    ...
    try {
      ...
      context.setProduces(ReflectRest.buildMediaType(clazz.getAnnotation(Produces.class),
          ...
          method.getAnnotation(Produces.class)));
  ...
  private void publishResource(ResourceBuilderContext context, List<Integer> types,
      Resource resource) {
      ...
      if (ra != null) {
        ...
        if (rs != null) {
          ...
          if (Logs.LOG.isLoggable(Level.FINE)) {
            Logs.LOG.log(Level.FINE, "resource {0} added with the http method {1}",
                new Object[] {resource.getPath(), ResourceArray.toString(type)});
          }
        }
      }
    }
  ...
  private void publishResourceInCollection(Resource resource, Integer type, ResourceArray ra,
      ResourceSelector rs) {
    ...
    if (Logs.LOG.isLoggable(Level.FINE)) {
      Logs.LOG.log(Level.FINE, "resource {0} already added with the http method {1}",
          new Object[] {resource.getPath(), ResourceArray.toString(type)});
    }
  }
  ...
  private boolean isNotPublicRestMethod(Method method) {
    return !Modifier.isPublic(method.getModifiers())
        && (method.isAnnotationPresent(Path.class) || method.isAnnotationPresent(Produces.class)
  ...
  private Map<Type, Type> buildGenericMap(Method method, Class<?> componentClass) {
    ...
    List<Class<?>> list =
        Reflect.getTypeArguments(method.getDeclaringClass(), (Class) componentClass, null);
